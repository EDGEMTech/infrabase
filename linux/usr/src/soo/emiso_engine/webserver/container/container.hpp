...
namespace container {
    ...
    public:
        ...
        shared_ptr<httpserver::http_response> render_GET(const httpserver::http_request &req) {
            ...
            for (const auto &arg : req.get_args()) {
                    ...
                    cout << "arg.first: " << string(arg.first) << endl;
                    cout << "arg.second: " << string(arg.second) << endl;
            ...
            if (info.empty()) {
                ...
                for (auto it = info.begin(); it != info.end(); ++it) {
                    ...
                    payload_json[idx]["ImageID"] = it->second.image;
                    ...
                    payload_json[idx]["NetworkSettings"]["Networks"]["bridge"]["NetworkID"]  = "NetworkID",
    ...
    public:
        ...
        shared_ptr<httpserver::http_response> render_POST(const httpserver::http_request &req) {
            ...
            string containerName = jsonData["name"].asString();

            // Remove the image version of the name
            size_t colonPosition = imageName.find(':');
            imageName = imageName.substr(0, colonPosition);

            cout << LOG_PREFIX "Create container based on '" << imageName << "' name" << endl;

            _daemon->container.create(imageName, containerName);

            // build the response
            payload_json["Id"] = containerName;
    ...
    public:
        ...
        shared_ptr<httpserver::http_response> render_POST(const httpserver::http_request &req) {
            ...
            cout << LOG_PREFIX "Create container based on '" << containerName << "' name" << endl;

            // == Retrieve container ID from the request path ==
            //int containerId =  __cxx11::stoi(req.get_arg("id"));

            _daemon->container.start(containerName);
