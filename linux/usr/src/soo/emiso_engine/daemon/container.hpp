...
namespace emiso {
    ...
    public:
        ...
        void info(map<int, ContainerInfo> &containerList);
        void info(ContainerInfo &info);

        int create(string imageName, string containerName);
        int start(string containerName);
        ...
        vector<string> retrieveLogs(unsigned containerId, unsigned lineNr);

    private:
        string meToDockerState(int meState);
        uint64_t createdTime();

        static map<int, ContainerId> _containersId;
